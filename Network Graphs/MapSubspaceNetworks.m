function [networks, area_label] = MapSubspaceNetworks(data, source_area_str, useProj, ndim)
...
for cur_rec = 1:nrec %loop through recordings
    ...
    for cur_motif = 1:nmotif %loop through motifs
        ...
        % Do MDS on the similarities
        rho2 = corr(cur_betas).^2;
        [Y, stress, disparities] = mdscale(rho2, 2);
        ...
        for i = 1:(N_areas - 1),
            for j = 1:ndim,
                plot(Y((i-1)*ndim + j, 1), Y((i-1)*ndim + j, 2), 'Color', colors{i}, 'Marker', markers{j}, 'MarkerSize', 14);
                hold on;
            end
        end

        % Do PCA
        [coeff,score,latent,tsquared,explained,mu] = pca(cur_betas);
        % Plot the distribution of PEV of each dimension
        figure; plot(cumsum(explained)); hold on;
        bar(explained);

        %Find the # of dimensions to explain a certain amount of variance
        cur_source_dims = [find(cumsum(explained) >= 80, 1, 'first') find(cumsum(explained) >= 90, 1, 'first') find(cumsum(explained) >= 95, 1, 'first') find(cumsum(explained) >= 99, 1, 'first')];
        fprintf('Needed %s dimensions to explain %s%% of the variance.\n', mat2str(cur_source_dims), mat2str([80 90 95 99]));

        % Plot the PCs themselves
        figure;
        for i = 1:(N_areas-1),
            plot([1:ndim] + (i-1)*(ndim+1), abs(coeff((i-1)*ndim + [1:ndim], 1:6))); 
            hold on;
        end
