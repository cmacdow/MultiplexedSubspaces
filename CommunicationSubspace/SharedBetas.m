function [rsq,matchdim] = SharedBetas(data,type,ndim,shufseed,matchdim)
        ...
        case 'B'
            for cur_motif = 1:size(data,2)
                for comp_motif = 1:size(data,2)
                    if cur_motif ~=comp_motif
                        idx = combvec(1:ndim,1:ndim);
                        %need to flip so that the majority of betas are
                        %positive to 'match' across motifs (since sign is
                        %arbitraty and the final sign is product of B and
                        %V'
                        ...
                        BB = data(comp_motif).rrr_B{cur_area}(:,1:ndim);
                        for i = 1:ndim
                            if sum(BB(:,i)>0)<sum(BB(:,i)<0)
                                BB(:,i) = BB(:,i)*-1;
                            end
                        end
                        if shufseed>1
                        ...
                        rtemp = arrayfun(@(n) corr(B(:,idx(2,n)),BB(shufidx(:,idx(1,n)),idx(1,n))), 1:size(idx,2),'UniformOutput',1);
                        [rsq(cur_area,cur_motif,comp_motif,1:ndim),matchdim(cur_area,cur_motif,comp_motif,1:ndim)] = arrayfun(@(n) max(rtemp(idx(2,:)==n)), unique(idx(2,:)),'UniformOutput',1);
                    end
                end
            end
        ...
        case 'V'
            for cur_motif = 1:size(data,2)
                for comp_motif = 1:size(data,2)
                    if cur_motif ~=comp_motif
                        idx = combvec(1:ndim,1:ndim);
                        B = data(cur_motif).rrr_V{cur_area}(:,1:ndim);
                        for i = 1:ndim
                            if sum(B(:,i)>0)<sum(B(:,i)<0)
                                B(:,i) = B(:,i)*-1;
                            end
                        end
                        BB = data(comp_motif).rrr_V{cur_area}(:,1:ndim);
                        for i = 1:ndim
                            if sum(BB(:,i)>0)<sum(BB(:,i)<0)
                                BB(:,i) = BB(:,i)*-1;
                            end
