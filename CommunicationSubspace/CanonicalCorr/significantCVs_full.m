function [a_full,b_full,U_full,V_full,rsq_xv,pval,rsq_xv_norm,aTheta,bTheta] = significantCVs_full(x,y,alpha,verbose)
...
%perform PCA
[~,xx,~,~,~] = pca(x,'NumComponents',20);
[~,yy,~,~,~] = pca(y,'NumComponents',20);

%run full cca
[a_full,b_full,r,U_full,V_full] = canoncorr(xx,yy); 

%get cross-validated R2 to test the generalization 
[rsq_xv,aTheta,bTheta] = xvalidateCCA(xx,yy,10);

%CCA on trial-permuted data for significance testing
%option 3: pCA
rsq_perm = cell(1,n_perm);
for i = 1:n_perm
   [~,yy_perm] = pca(squeeze(y_perm(:,:,i)),'NumComponents',20);
   [~,~,rsq_perm{i}] = canoncorr(xx,yy_perm);
end
