...
%% plot and example between two motif in the visual cortex
cur_a = 8;
m = [5,7];

%are they the same (no) 
% [~,area_label] = LoadVariable(data,'rrr_dim',[]);
% rrrB = arrayfun(@(n) LoadVariable(data,'rrr_B',area_label{cur_a},n),1:29,'UniformOutput',0);
% rrrV = arrayfun(@(n) LoadVariable(data,'rrr_V',area_label{cur_a},n),1:29,'UniformOutput',0);

%are they completely different (no)
%plot their PEV per dimension
y= NaN(10,6);
for cur_d = 1:10
   xbeta= arrayfun(@(n) squeeze(beta{n}(1).rsq(cur_a,:,:,cur_d)), 1:size(beta,2),'UniformOutput',0);
   xbeta = cat(3,xbeta{:});
   y(cur_d,:) = xbeta(m(1),m(2),:);
end
x=y';
x(x<0)=0;
figure; hold on;
...
title({'Comparing similarity of','subspaces for motif 5 and 7'},'Fontweight','normal')
fp.FormatAxes(gca);  box on; grid on
fp.FigureSizing(gcf,[3 2 6 6],[10 10 10 10])

%actually show their first dimension - and how related it is - | compare
%that to a later dimension


% Get a summary measure of shared dimensioanlity across all dimensions
rrr_mdl = LoadVariable(data,'rel_performance','VIS');
y(y<0)=0;
z = squeeze(rrr_mdl(:,m(1),1:10));
...
for cur_a = 1:8   
    ...
    for cur_d = 1:10
       xbeta= arrayfun(@(n) squeeze(beta{n}(1).rsq(cur_a,:,:,cur_d)), 1:size(beta,2),'UniformOutput',0);
       xbeta = cat(3,xbeta{:});
       y(cur_d,:,:) = reshape(xbeta,14*14,6)';
    end    
    rrr_mdl = LoadVariable(data,'rel_performance',area_label{cur_a});
    y(y<0)=0;
    z = squeeze(rrr_mdl(:,m(1),1:10));
end
